$(document).ready(function() {
		...
		if (fileToLoad) {
			...
			reader.onload = function(fileLoadedEvent) {
				...
				for (i = 0; i < obj.length; i++) { 
					...
					$x[i] = obj[i].x;
					$y[i] = obj[i].y;
					$duration[i] = obj[i].duration;
					$keyboardtype[i] = obj[i].keyboardtype;
					$leftclick[i] = JSON.parse(obj[i].leftclick);
					$middleclick[i] = JSON.parse(obj[i].middleclick);
					$rightclick[i] = JSON.parse(obj[i].rightclick);
					$scrollleft[i] = obj[i].scrollx;
					$scrolltop[i] = obj[i].scrolly;
					$windowwidth[i] = obj[i].windowwidth;
					$windowheight[i] = obj[i].windowheight;
				}
			};
	...
	$('#generate-mouse-trail').click(function() {
		...
		$.each($y, function(index, value) {
			...
			setTimeout(function() {
if($name[index]=="Bolortsetseg uuganbayar"){
				$('canvas').drawArc({
...
if($name[index]=="Bolortsetseg uuganbayar"){
			var dataPoint = {
				x: $x[index]-180, // x coordinate of the datapoint, a number 
