<?php
	...
	$check_table_exist = mysqli_query($conn, "SELECT id FROM pagetracking");
	if(empty($check_table_exist)) {
                $create_table = "CREATE TABLE pagetracking (
                ...
                if ($conn->query($create_table) === TRUE) {
					echo "Table pagetracking created successfully";
				} else {
					echo "Error creating table: " . $conn->error;
				}
	}
	
	$sql = "INSERT INTO pagetracking (name,email,date,mousemove,duration,current_url,ip_address,leftclick,rightclick,middleclick,keyboardtype,scroll,zoom,active,inactive) VALUES ('$name', '$email', '$date', '$mousemove', '$duration', '$current_url', '$ip_address', '$leftclick', '$rightclick', '$middleclick', '$keyboardtype', '$scroll', '$zoom', '$active', '$inactive')";
